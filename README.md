...
这是一个可以直接扩展的 ICPC 代码板仓库。把 `.cpp` 文件按算法分类放进目录，构建脚本会按路径自然排序，并生成适合 A4 横向黑白打印的 `main.pdf`。
...
板子中无需写 `main()`；建议保持每行不超过 `codebook.toml` 中的 `warn_line_length`，双栏打印会更清楚。`@...` 元数据只进入标题信息条，不会在代码区重复打印。
...
在 `codebook.toml` 修改封面标题、队名、赛季、忽略目录和行长警告阈值。排版细节在 `codebook.typ` 中，包括 A4 横向双栏、目录、章节编号、PDF 书签、页眉、页码、灰色行号及纯黑代码。
